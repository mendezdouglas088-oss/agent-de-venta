...
export function ProfileSidebar({ activeChat, open = false, onClose }) {
  return (
    // Ocupa espacio real en el layout (no es un overlay): al abrirse/cerrarse
    // empuja/libera espacio para ChatView, su hermano en el flex del Inbox.
    <div
      className={`h-full shrink-0 overflow-hidden border-l border-neutral-200 bg-white transition-[width] duration-300 ease-in-out dark:border-neutral-700 dark:bg-neutral-900 ${
        open ? "w-72" : "w-0 border-transparent"
      }`}
    >
      <div className="flex h-full w-72 flex-col overflow-y-auto px-5 py-5">
